    ...
    if [ "$clone_backend" == y ] ; then
        ...
        if [ "$GIT_BACKEND_LOCAL" != n ] ; then
            ...
            if [ ! -d "$GIT_BACKEND_LOCAL" ]; then
                echo "${yellow}Installing necessarily PHP modules (php-zip, ext-simplexml, ext-curl, ext-mbstring, ext-gmp, ext-libsodium)${clear}"
                sudo apt-get install unzip php-zip php-simplexml php-curl php-mbstring php-gmp php-libsodium
