...
#' @param h.ratio Whether to return the suggested width/height ratio to multiple plot height by.
#' Suggestion: x11(width = 7, height = 7 * h.ratio)
...
corrplot <-
            ...
            ...)
  {
    ...
    if (order != "original") {
      ord <-
        corrMatOrder(corr, order = order, hclust.method = hclust.method)
      corr <- corr[ord, ord]
    }
    ...
    apply_mat_filter <- function(mat) {
      x <- matrix(1:n * m, n, m)
      switch(type, upper = mat[row(x) > col(x)] <-
               Inf, lower = mat[row(x) <
                                  col(x)] <-
               Inf)
      if (!diag) {
    ...
    if (!add) {
      ...
      for (i in 1:50) {
        ...
        xlim <- c(
          m1 - 0.5 - xlabwidth * (grepl("l", tl.pos) |
                                    grepl("d", tl.pos)) - laboffset
          ,
        ...
        ylim <- c(
          ...
          n2 + 0.5 + laboffset +
            ylabwidth * abs(sin(tl.srt * pi / 180)) * (grepl("d", tl.pos) |
                                                         grepl("t", tl.pos))
        ) + c(-0.15, 0.2)
        ...
        laboffset.tmp <-
          strwidth("W", cex = tl.cex) * tl.offset
        ...
        if (i == 50) {
          warning(
            c(
              "Not been able to calculate text margin, ",
      ...
      if (.Platform$OS.type == "windows") {
        ...
        if (round(diff(ylim) / diff(xlim), digits = 1) != 1.0) {
          message(paste0(
            "Suggested windows size: height = width * ",
            round(diff(ylim) / diff(xlim), digits = 2)
          ))
        }
      }
      if (h.ratio) {
    ...
    if (any(is.na(corr)) && is.character(na.label)) {
      ...
      if (na.label == "square") {
        ...
        stop(paste(
          "Maximum number of characters for NA label is:",
          NA_LABEL_MAX_CHARS
        ))
      }
    }
    ...
    if (method == "shade" && plotCI == "n") {
      ...
      SHADE.dat <- matrix(na.omit(unlist(lapply(
        pos_corr2,
        shade.dat
      ))), byrow = TRUE, ncol = 4)
      segments(SHADE.dat[, 1],
               SHADE.dat[, 2],
               SHADE.dat[,
                         3],
               SHADE.dat[, 4],
    ...
    if (!is.null(addCoef.col) && method != "number") {
      text(
        ...
        col = addCoef.col,
        labels = round((DAT -
                          int) * ifelse(addCoefasPercent, 100, 1) /
                         zoom,
                       number.digits
        ),
        cex = number.cex,
