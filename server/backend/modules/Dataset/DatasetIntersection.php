...
class DatasetIntersection {
     ...
     * @param  [boolean] $extraction Are we doing mulset on dataset
     * @param  [boolean] $backwardSelection Are we doing Recursive feature Elimination
     * @param  [boolean] $mainDatasetImputation Are we doing imputation on main dataset
     * @return [string]          [path to the file containing results]
     */
    public function generateDataPresets($filePath, $outcome, $features, $extraction, $backwardSelection, $mainDatasetImputation = false) {
        ...
        if($extraction === false && $mainDatasetImputation === true){
            $allowNAValues = true;
        }
        ...
        if (count($records) > 0) {
            foreach ($records as $sampleID => $record) {
                ...
                foreach ($record as $recordID => $recordValue) {
                    ...
                    if (!isset($samples[$sampleID][$recordID]) && $isNumeric === true || $allowNAValues === true) {
